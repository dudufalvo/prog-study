...
void Jogo::adicionaTema(){
    ...
    do {
        ...
        if (palavra!="FIM") {
            strcpy(string_C, palavra.c_str());
            NovoTema.write((char*) string_C, sizeof(string_C));
...
void Jogo::editaTema(){
    ...
    count = 0;
    do{
        coletaPalavras.seekg(count * sizeof(string_C), ios::beg);
        coletaPalavras.read((char *) &string_C, sizeof(string_C));
        if(coletaPalavras.gcount() == sizeof(string_C)){
            cout << count+1 << "-" << string_C << endl;
            count++;
            palavrasListadas.insereItem(string_C);
            }

    }while(coletaPalavras.gcount() == sizeof(string_C));
    ...
    case 2:
        ...
        if(!(palavrasListadas.removeItem(escolha))){
            cout << "ERRO REMOVENDO PALAVRA" << endl;
        }
        palavrasListadas.escreveFicheiro(tema);
    ...
    case 3:
        ...
        coletaPalavras.open(tema+".dat", ios::binary);
        coletaPalavras.seekg((escolha-1) * sizeof(string_C), ios::beg);
        coletaPalavras.read((char *) &string_C, sizeof(string_C));
        coletaPalavras.close();
        cout << string_C << endl;
        cout << "ENTRE O QUE QUER QUE A PALAVRA SEJA: ";
        cin >> string_C;
...
void Jogo::avanca_jogo(){
    ...
    do{
        ...
        coletaPalavra.seekg(random * sizeof(string_C), ios::beg);
        coletaPalavra.read((char *) string_C, sizeof(string_C));
        
        if(coletaPalavra.gcount() != sizeof(string_C)){
